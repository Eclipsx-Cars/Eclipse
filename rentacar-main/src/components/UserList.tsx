...
const UserList: React.FC = () => {
    ...
    const verifyDriver = async (user: User) => {
        try {
            const response = await axios.put(
                `${process.env.REACT_APP_API_URL}/api/users/${user.id}/verifyDriver`
            );
            if (response.status === 200) {
                setUsers((prevState) =>
                    prevState.map((u) => (u.id === user.id ? response.data : u))
                );
            }
        } catch (error) {
                    ...
                    <tr className="bg-gray-600 text-gray-300" key={user.id}>
                        ...
                        <td className="border px-4 py-2">
                            ...
                            {user.isDriver && !user.isVerifiedDriver && (
                                <button className="mx-1 bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded" onClick={() => verifyDriver(user)}>Verify Driver</button>
                            )}
